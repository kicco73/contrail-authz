...
import org.ow2.contrail.authorization.cnr.old.pep.PEP_OLD;
import org.ow2.contrail.authorization.cnr.old.pep.PEP_callout_OLD;
...
public class SimpleTestOld {
	...
	try {
	    ...
	    PEP_OLD pep = new PEP_OLD(pdp_endpoint, pep_port, pep_host);

	    PEP_callout_OLD pep_callout = new PEP_callout_OLD(pep);
		...
		// cert = sc.getCertFromStream(new FileInputStream(certPos));
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	    ...
	    List<PepRequestAttribute_OLD> req = pep_callout.getAttributesFromCertificate(cert);
	    PepRequestAttribute_OLD res = new PepRequestAttribute_OLD("urn:oasis:names:tc:xacml:1.0:resource:resource-id", type, "aa", issuer,
		    UconCategory.RESOURCE);
	    req.add(res);
	    PepRequestAttribute_OLD act = new PepRequestAttribute_OLD("urn:contrail:vep:action:id", type, "aa", issuer, UconCategory.ACTION);
	    req.add(act);

	    // TestManagerCallout testManager = new TestManagerCallout(serverIp);
	    // testManager.setVerbosity(1);
	    //
	    // testManager.startIdP();

	    System.out.println("Test: send try access");
	    boolean accessDecision = pep_callout.tryaccess(req);
