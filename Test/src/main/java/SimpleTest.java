...
public class SimpleTest {
	...
	public void run(String[] args) throws InterruptedException {
		try {
			String pdp_endpoint = "http://" + serverIp
					+ "/axis2/services/UconWs";
			...
			// prepare request example request (at least SUBJECT - ACTION -
			// OBJECT)
			...
			PepRequestAttribute res = new PepRequestAttribute(
					"urn:oasis:names:tc:xacml:1.0:resource:resource-id", type,
					"aa", issuer, UconCategory.RESOURCE);
			req.add(res);
			PepRequestAttribute act = new PepRequestAttribute(
					"urn:contrail:vep:action:id", type, "aa", issuer,
					UconCategory.ACTION);
			req.add(act);
			...
			if (accessDecision) {
				...
				urlBase += "users/" + uuid + "/applications/" + appUuid
						+ "/revoke";
