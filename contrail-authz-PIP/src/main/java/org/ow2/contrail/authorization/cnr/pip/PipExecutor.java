...
public class PipExecutor {
	...
	public Element attributeQuery(Element xmlRequest) throws XacmlSamlException {
		// get the requests
		log.debug("{} [KMcC;)] attributeQuery() - xmlRequest = {}", logTag,
				xmlRequest);
		List<PipDataEntity> requests = PipDataEntity
				.getInstanceFromUconRequest(serviceContext, xmlRequest, null);

		// ask for attributes //CHECKME ALWAYS????? TODO it could be optimized
		// (if I had some attributes with auto-update)
		log.debug(
				"{} [KMcC;)] attributeQuery() - calling contactIdentityProvider()",
				logTag);
		PipDataEntity[] data = contactIdentityProvider(requests);
		// compose the response
		Element response = PipDataEntity.composeXacmlSet(serviceContext,
		 true, data);

		return response;
	}

	public Element addSubscription(Element request) throws XacmlSamlException,
			SQLException {
		...
		List<PipDataEntity> dataRequests = PipDataEntity
				.getInstanceFromUconRequest(serviceContext, request, subscriber);
		...
		List<PipDataEntity> notUpdated = db.insertSubscriptionIfOwnerExist(
				dataRequests, subscriber);
		// ask them to identity provider
		log.debug(
				"{} [KMcC;)] addSubscription() - calling contactIdentityProvider()",
				logTag);
	...
	public Element removeSubscription(Element request) throws SQLException,
			XacmlSamlException {
	...
	public Element triggeredUpdate() throws SQLException, XacmlSamlException,
			AxisFault {
		...
		log.debug(
				"{} [KMcC;)] triggeredUpdate() - calling contactIdentityProvider()",
				logTag);
	...
	private Element getBooleanResponse(boolean b) {
		return (b) ? UconConstantsPip.PIP_ATTRIBUTE_SUBSCRIBE_OK
				: UconConstantsPip.PIP_ATTRIBUTE_SUBSCRIBE_NOT_OK;
	}

	// TODO: parallelize it!!! (preserving the order)

	private PipDataEntity[] contactIdentityProvider(List<PipDataEntity> requests)
		...
		for (PipDataEntity query : requests) {
			log.debug(
					"{} [KMcC;)] contactIdentityProvider([]) - calling contactIdentityProvider()",
					logTag);
	...
	private PipDataEntity contactIdentityProvider(PipDataEntity query)
			throws XacmlSamlException {
		try {
			String url = getIdentityProviderUrl(query);
			Element response = communicator.queryIdentityProvider(url,
					query.getSamlAttributeQuery());
			// TODO: auto update: where I can get this value? I know it
			// statically, or Identity Provider could give me?
			log.debug("{} [KMCc;)] contactIdentityProvider() got response: {}",
					logTag, response);
			log.debug(
					"{} [KMCc;)] contactIdentityProvider() serviceContext: {}",
					logTag, serviceContext);
			log.debug("{} [KMCc;)] contactIdentityProvider() category: {}",
					logTag, query.getCategory());
			log.debug("{} [KMCc;)] contactIdentityProvider() subscriber: {}",
					logTag, query.getSubscriber());
			PipDataEntity result = PipDataEntity.getInstanceFromSaml(
					serviceContext, response, query.getCategory(),
					query.getSubscriber(), false);
			return result;
		} catch (SOAPException e) {
			throw new XacmlSamlException(e); // TODO: error message?
		} catch (Exception e) {
	...
	private void performUpdateNotification(List<PipDataEntity> toUpdate)
			throws XacmlSamlException, AxisFault {
		log.info("{} perform the update notification to the ucon services");

		// compose the message
		Element msg = PipDataEntity.composeXacmlSet(serviceContext, false,
				(PipDataEntity[]) toUpdate.toArray());
	...
	private String getSenderAddress() {
		...
		String address = (String) messageContext
				.getProperty(MessageContext.REMOTE_ADDR); // the actual client
															// address
		return String.format("http://%s:8080/axis2/services/UconWs", address); // TODO:
																				// how
																				// to
																				// get
																				// the
